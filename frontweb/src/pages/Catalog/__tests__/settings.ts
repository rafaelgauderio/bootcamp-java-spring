...
// dados re requisição do backend
const findAllProductsJson = {
    "content": [
        {
            "id": 1,
...
// configuração de um servidor mockando o backend
// para o teste não depender do backend real

export const serverBackend = setupServer(

    rest.get(`${BASE_URL}/products`, (request, response, context) => {
        return response(
            context.json(findAllProductsJson),
            context.status(200),
        );
    })
